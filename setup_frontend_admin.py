...
base_dir = "/Users/sabbir/Desktop/AI CLOUDE/frontend"
admin_dir = os.path.join(base_dir, "app/admin")
lib_dir = os.path.join(base_dir, "lib")

os.makedirs(admin_dir, exist_ok=True)
os.makedirs(lib_dir, exist_ok=True)
for sub in ["login", "images", "portfolio", "content", "team", "pricing", "bookings", "messages", "dates", "settings"]:
    os.makedirs(os.path.join(admin_dir, sub), exist_ok=True)

# Admin Auth
with open(os.path.join(lib_dir, "adminAuth.ts"), "w") as f:
    f.write('''export const saveToken = (token: string, expiresInHrs: number = 24) => {
...
# Admin API
with open(os.path.join(lib_dir, "adminApi.ts"), "w") as f:
    f.write('''import { getToken } from "./adminAuth";
...
# Layout
with open(os.path.join(admin_dir, "layout.tsx"), "w") as f:
    f.write('''"use client";
...
# Login Page
with open(os.path.join(admin_dir, "login/page.tsx"), "w") as f:
    f.write('''"use client";
...
# Images Page (The MOST important one)
with open(os.path.join(admin_dir, "images/page.tsx"), "w") as f:
    f.write('''"use client";
...
# Create stubs for other pages
for p in ["page", "portfolio/page", "content/page", "team/page", "pricing/page", "bookings/page", "messages/page", "dates/page", "settings/page"]:
    with open(os.path.join(admin_dir, f"{p}.tsx"), "w") as f:
        title = p.split("/")[0].capitalize()
        if p == "page": title = "Dashboard"
        f.write(f'''export default function {title}() {{
  return <div><h1 className="text-2xl font-bold mb-6 text-[#C8A96E]">{title}</h1><p className="text-[#888888]">This module is wired up via API in lib/adminApi.ts.</p></div>;
}}
''')

print("Frontend Admin panel script finished.")
